...
import { sizedClone } from "./assets.js";
...
export function createMarina(area, waterY = 0.5, heightAt = () => 0, assets = {}) {
  ...
  for (let a = along0 + 6; a <= along1 - 6; a += STEP) {
    ...
    for (const side of [-1, 1]) {
      ...
      let y;
      if (assets.sailboat && n % 4 === 1) {
        // Velero real (CC0) para dar variedad.
        y = new THREE.Group();
        y.add(sizedClone(assets.sailboat, size.len * 1.5));
      } else {
        y = makeYacht(size); // yate de motor (procedural)
      }

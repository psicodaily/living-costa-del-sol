...
// Cinta de calle que sigue el relieve. UV en metros/UV_TILE para texturizar.
const UV_TILE = 4;
function ribbonGeometry(path, width, yOffset, heightAt) {
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    uv.push(positions[i] / UV_TILE, positions[i + 2] / UV_TILE);
  }
...
export function buildGround(data, heightAt = () => 0, assets = {}) {
  ...
  const sea = new THREE.Mesh(seaGeo, new THREE.MeshStandardMaterial({ color: 0x16566f, roughness: 0.12, metalness: 0.4, envMapIntensity: 1.4 }));
  ...
  // Materiales con TEXTURA PBR (CC0) si están disponibles; si no, color liso.
  const pbrMat = (texset, fallback, rough) => texset
    ? new THREE.MeshStandardMaterial({ ...texset, roughness: 1, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: -2, polygonOffsetUnits: -2 })
    : surfMat(fallback, rough);
  const sidewalks = mergedMesh(sidewalkGeos, pbrMat(tex.paving, 0xbdb9af));
  if (sidewalks) group.add(sidewalks);
  const roads = mergedMesh(roadGeos, pbrMat(tex.asphalt, 0x3b3f47, 0.95));

...
import { buildGround, bounds as groundBounds, SEA_Y } from "./ground.js";
...
export function createRealCity(data, assets = {}) {
  ...
  // MAR animado: agua con olas y reflejos (Water). Cubre un área amplia al nivel
  // del mar; las zonas bajas (costa, dársenas excavadas) se ven de agua.
  const gb = groundBounds(data);
  const seaSize = Math.max(gb.maxX - gb.minX, gb.maxZ - gb.minZ) * 3 + 480;
  const waterNormals = new THREE.TextureLoader().load("/textures/waternormals.jpg", (t) => { t.wrapS = t.wrapT = THREE.RepeatWrapping; });
  const water = new Water(new THREE.PlaneGeometry(seaSize, seaSize), {
    textureWidth: 512, textureHeight: 512,
    waterNormals,
    sunDirection: new THREE.Vector3(0.3, 0.85, 0.2).normalize(),
    sunColor: 0xfff2dd,
    waterColor: 0x0b4a63,
    distortionScale: 2.6,
    fog: true,
  ...
  water.position.set((gb.minX + gb.maxX) / 2, SEA_Y, (gb.minZ + gb.maxZ) / 2);
  group.add(water);
  ...
  return { group, cityBoxes, bounds: { minX, maxX, minZ, maxZ }, heightAt, water };
}

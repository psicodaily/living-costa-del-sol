...
import { instancePrototype } from "./assets.js";
...
// ---------- Distrito ----------
export function createPuertoBanus(buildings, data, heightAt = () => 0, assets = {}) {
  ...
  // Muros: material multi-grupo [tejado, pared]. La pared usa la textura de
  // ventanas (color) + el relieve real de plaster (normal/rough) para grano.
  const wallTex = (assets.tex && assets.tex.wall) || {};
  add(wallGeos, [
    new THREE.MeshStandardMaterial({ color: 0xa85a36, roughness: 0.9, side: THREE.DoubleSide }), // teja
    new THREE.MeshStandardMaterial({
      color: 0xefe9dc, map: windowTex, normalMap: wallTex.normalMap || null,
      roughnessMap: wallTex.roughnessMap || null, roughness: 0.95, side: THREE.DoubleSide,
    }), // pared crema con grano de plaster
  ]);
  ...
  if (marina) {
    ...
    let kind = 0;
    const palmT = []; // transformaciones para instanciar las palmeras
    ...
    for (let i = 0; i < ring.length; i++) {
      ...
      for (let s = 0; s < steps; s++) {
        ...
        const k = (kind++) % 4;
        if (k === 0 || k === 1) {
          // Palmera: si hay modelo CC0, lo instanciamos; si no, procedural.
          if (assets.palm) {
            const rot = (kind * 1.3) % (Math.PI * 2);
            const sc = 0.85 + ((kind * 7) % 5) * 0.07;
            palmT.push(new THREE.Matrix4().compose(
              new THREE.Vector3(x, y, z),
              new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), rot),
              new THREE.Vector3(sc, sc, sc)
            ));
          } else {
            const pm = makePalm(); pm.position.set(x, y, z); group.add(pm);
          }
        } else if (k === 2) {
          const lp = makeLamp(); lp.position.set(x, y, z); group.add(lp);
        } else {
          const tc = makeTrashCan(); tc.position.set(x, y, z); group.add(tc);
        }
      }
    }
    if (assets.palm && palmT.length) group.add(instancePrototype(assets.palm, palmT, 7.5));

...
const CENTER = N / 2; // cruce central = rotonda (los coches lo rodean)
const rc = (i) => -CITY_HALF + i * CELL;
// Un cruce es transitable si está en la rejilla y NO es la rotonda central.
const nodeOk = (i, j) => i >= 0 && i <= N && j >= 0 && j <= N && !(i === CENTER && j === CENTER);
...
export function createTraffic(scene, count = 8) {
  ...
  for (let k = 0; k < count; k++) {
    ...
    let ix = Math.floor(rand() * (N + 1));
    let iz = Math.floor(rand() * (N + 1));
    if (ix === CENTER && iz === CENTER) iz = (iz + 1) % (N + 1); // no empezar en la rotonda
...
function pickInitialDir(ix, iz, rand) {
  const all = [
    ...
    { di: 0, dj: 1 },
    { di: 0, dj: -1 },
  ].filter((d) => nodeOk(ix + d.di, iz + d.dj));
...
function chooseDir(ix, iz, dir, rand) {
  ...
  const opts = [];
  if (nodeOk(ix + straight.di, iz + straight.dj)) opts.push(straight, straight); // peso doble
  if (nodeOk(ix + left.di, iz + left.dj)) opts.push(left);
  if (nodeOk(ix + right.di, iz + right.dj)) opts.push(right);

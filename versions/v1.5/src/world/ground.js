...
export function createGround(scene) {
  ...
  // Suelo base (hierba) por debajo de todo, visible en las afueras.
  const groundSize = CITY.blocks * CELL + 260;
  ...
  const edgeMat = new THREE.MeshStandardMaterial({ color: 0xf0f0f0, roughness: 0.6 });
  ...
  const matV = new THREE.MeshStandardMaterial({ map: crossTex, transparent: true, roughness: 0.7 });
  const matH = new THREE.MeshStandardMaterial({ map: crossTexH, transparent: true, roughness: 0.7 });
...
function addLine(group, geo, x, z, mat, dashed, dashMap) {
  const material = dashed
    ? new THREE.MeshStandardMaterial({ map: dashMap, transparent: true, roughness: 0.6 })
    : mat;

...
function animate() {
  ...
  if (mode === "foot") {
    ...
    // Choque: si el coche es empujado fuera de un obstáculo yendo rápido, daña.
    const push = obstacles.resolve(car.group.position, 2.0);
    if (push > 0.05 && Math.abs(car.getSpeed()) > 7) {
      car.applyImpact(Math.min(30, Math.abs(car.getSpeed()) * 0.8));
      shakeTime = 0.28;
    }

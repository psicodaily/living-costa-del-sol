...
import { createWindowTexture, createWindowEmissiveTexture, tiled } from "./textures.js";
...
export function createBuildings(scene) {
  ...
  const emissiveBase = createWindowEmissiveTexture();
  ...
  const boxes = [];
  // Materiales de fachada (para encender las ventanas de noche).
  ...
  forEachBlock((cx, cz) => {
    ...
    for (const [ox, oz] of spots) {
      ...
      const repX = Math.max(2, Math.round(w / 4));
      const repY = Math.max(2, Math.round(h / 4));
      const winTex = tiled(windowBase, repX, repY);
      const emiTex = tiled(emissiveBase, repX, repY);
      ...
      const mat = new THREE.MeshStandardMaterial({
        ...
        emissive: 0xffffff,
        emissiveMap: emiTex,
        emissiveIntensity: 0, // de día apagado; de noche lo sube dayNight
      });
  ...
  scene.add(buildings);
  return { group: buildings, boxes, windowMaterials };
}

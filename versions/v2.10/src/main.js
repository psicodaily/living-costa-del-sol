...
async function init() {
  ...
  const [pbx, pbz] = puertoBanusTarget(data);
  const [spx, spz] = findSpawn(data, obstacles, pbx, pbz);
  ...
  const life = createCityLife(scene, data.roads, { cars: 8, peds: 14 }, heightAt);
...
// Devuelve el punto de Puerto Banús (centroide de su marina) para aparecer ahí.
function puertoBanusTarget(data) {
  ...
  if (!area && marinas.length) {
    // La marina más cercana al punto de referencia de Puerto Banús.
  ...
  if (area) return centroid(area.polygon);
  return [-3533.8, 1858.2]; // referencia fija si no hay datos de marina
}

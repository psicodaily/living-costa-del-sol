...
export function buildBuildings(buildings, heightAt = () => 0) {
  ...
  for (const b of buildings) {
    ...
    // Apoyar el edificio en el punto MÁS BAJO de su base, así nunca "flota" en
    // las cuestas (la parte de arriba de la cuesta se entierra un poco y no se ve).
    const baseY = Math.min(
      heightAt(bb.min.x, bb.min.z),
      heightAt(bb.max.x, bb.min.z),
      heightAt(bb.min.x, bb.max.z),
      heightAt(bb.max.x, bb.max.z),
      heightAt(cx, cz)
    ) - 0.5;

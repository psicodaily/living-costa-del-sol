...
// Ventanas iluminadas (para el mapa emisivo): ventanas claras sobre fondo negro.
// De noche, con emissiveIntensity > 0, hacen que las ventanas "se enciendan".
export function createWindowEmissiveTexture() {
  const size = 256;
  const c = makeCanvas(size);
  const ctx = c.getContext("2d");
  ...
  for (let y = 0; y < cells; y++) {
    for (let x = 0; x < cells; x++) {
      const px = x * cell + (cell - winW) / 2;
      const py = y * cell + (cell - winH) / 2;
      // Algunas encendidas (cálidas) y otras apagadas, para dar variedad.
      const lit = (x * 3 + y * 7) % 4 !== 0;
      ctx.fillStyle = lit ? "#ffdc92" : "#140d02";
      ctx.fillRect(px, py, winW, winH);
    }
  }
  return finish(c);
...
// Línea de carril discontinua: trazo blanco que, al repetirse, deja huecos.

...
export function buildGround(data, heightAt = () => 0) {
  ...
  // Suelo base: malla con RELIEVE real (cuestas).
  const segX = 200;
  const segZ = Math.max(40, Math.round(segX * (d / w)));
  const terrainGeo = new THREE.PlaneGeometry(w, d, segX, segZ);
  terrainGeo.rotateX(-Math.PI / 2);
  const tp = terrainGeo.attributes.position;
  for (let i = 0; i < tp.count; i++) {
    tp.setY(i, heightAt(tp.getX(i) + cx, tp.getZ(i) + cz) - 0.05);
  }

...
export function createSky(scene, opts = {}) {
  // Opciones (escala). Por defecto, los valores de la ciudad de cuadrícula;
  // para el mapa real se pasan valores mucho mayores.
  const radius = opts.radius ?? 600;
  const fogNear = opts.fogNear ?? 140;
  const fogFar = opts.fogFar ?? 460;
  const shadowHalf = opts.shadowHalf ?? 180;
  const shadowFar = opts.shadowFar ?? 400;

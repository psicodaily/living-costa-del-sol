...
export function createVehicle(scene, opts = {}) {
  ...
  let condition = 100;
  let smokeT = 0;
  let shoveVX = 0; // empuje recibido (cuando otro vehículo lo embiste)
  let shoveVZ = 0;
  ...
  // Para vehículos aparcados: aplica el empuje recibido y lo va frenando.
  function passiveUpdate(delta) {
    if (shoveVX !== 0 || shoveVZ !== 0) {
      group.position.x += shoveVX * delta;
      group.position.z += shoveVZ * delta;
      const friction = Math.max(0, 1 - 5 * delta);
      shoveVX *= friction;
      shoveVZ *= friction;
      if (Math.abs(shoveVX) < 0.05 && Math.abs(shoveVZ) < 0.05) {
        shoveVX = 0;
        shoveVZ = 0;
      }
      group.position.x = THREE.MathUtils.clamp(group.position.x, -BOUND, BOUND);
  ...
  // Choque contra pared (edificio): abolla y frena en seco.
  ...
  return {
    ...
    applyImpact,
    shove: (vx, vz) => {
      // Se fija (no se acumula) para que no salga disparado al estar en contacto.
      shoveVX = vx;
      shoveVZ = vz;
    },
    dent: (amount) => {

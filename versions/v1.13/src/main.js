...
import { createInteractions } from "./physics/interactions.js";
import { createTraffic } from "./traffic/traffic.js";
import { createWanted } from "./gameplay/wanted.js";
import { createPolice } from "./police/police.js";
import { createRadio } from "./audio/radio.js";
import { createMissions } from "./missions/missions.js";
import { createShops } from "./gameplay/shops.js";
import { createShopPanel } from "./ui/shopPanel.js";
import { saveGame, loadGame, clearGame } from "./gameplay/save.js";
import { getState, setState, addMoney, setHealth } from "./state/gameState.js";
...
// Tráfico con IA: coches por la rejilla + semáforos (v1.11).
const traffic = createTraffic(scene, 8);

// Choques con realismo: vehículos (aparcados y tráfico), árboles y peatones.
const interactions = createInteractions({ fleet, trees: world.trees, npcs, traffic });

// Policía y nivel de búsqueda (v1.12).
const wanted = createWanted();
const police = createPolice(scene, 4);
const starsEl = createStars();

// Alma: radio, misiones, tiendas y guardado (v1.13).
const radio = createRadio();
const shops = createShops(scene);
const dialogEl = createDialog();
const radioEl = createRadioHud();
let dialogTimer = 0;
let radioTimer = 0;
let autosaveT = 0;
...
const missions = createMissions(scene, {
  showDialog,
  onReward: (amount) => addMoney(amount),
});

const shopPanel = createShopPanel({
  getMoney: () => getState().money,
  onBuyHeal: () => {
    if (getState().money >= 50) {
      addMoney(-50);
      setHealth(100);
      return true;
    }
...
function doSave() {
  ...
  saveGame({
    money: s.money,
    health: s.health,
    px: player.group.position.x,
    pz: player.group.position.z,
  });
...
// La cámara solo responde al ratón si no hay pausa, mapa ni tienda abiertos.
function updateCameraActive() {
  followCamera.setActive(!pause.isPaused() && !minimap.isBigOpen() && !shopPanel.isOpen());
}
...
onPress("KeyE", () => {
  ...
  // A pie: la tienda tiene prioridad si estás al lado.
  if (shopPanel.isOpen()) {
    shopPanel.close();
    return;
  }
  if (shops.nearest(player.group.position, 4)) {
    shopPanel.open();
    return;
  }
...
function animate() {
  ...
  // En pausa, con el mapa o la tienda abiertos: congela pero sigue dibujando.
  if (pause.isPaused() || minimap.isBigOpen() || shopPanel.isOpen()) {
    postfx.render();
    return;
  }
  ...
  if (mode === "foot") {
    ...
    // Choques con coches aparcados, árboles y peatones (suben la búsqueda).
    interactions.driveCollisions(
      vehicle,
      () => {
        shakeTime = Math.max(shakeTime, 0.22);
      },
      (amount) => wanted.addCrime(amount)
    );
  ...
  // Peatones: reaccionan a la posición activa; "amenaza" si conduces o corres.
  const threat = mode === "drive" || keys.run;
  npcs.update(delta, activePos, threat);
  traffic.update(delta, activePos); // tráfico de la IA + semáforos

  // Nivel de búsqueda y policía (v1.12).
  wanted.update(delta);
  const stars = wanted.getStars();
  police.update(delta, activePos, stars);
  updateStars(stars);

  // Misiones, avisos y guardado automático (v1.13).
  missions.update(delta, activePos);
  if (dialogTimer > 0) {
    dialogTimer -= delta;
    if (dialogTimer <= 0) dialogEl.style.opacity = "0";
  }
  if (radioTimer > 0) {
    radioTimer -= delta;
  ...
  const mt = missions.getTarget();
  if (mt) dots.push({ x: mt.x, z: mt.z, color: "#4aa3ff", size: 4 }); // misión
  for (const s of shops.shops) dots.push({ x: s.x, z: s.z, color: "#6fffa0", size: 3 }); // tiendas
...
// Cargar partida guardada (dinero, salud, posición) si existe.
const saved = loadGame();
if (saved) {
  setState({ money: saved.money ?? 2500, health: saved.health ?? 100 });
  if (typeof saved.px === "number" && typeof saved.pz === "number") {
    player.group.position.set(saved.px, 0, saved.pz);
  }
  showDialog("Partida cargada.");
}
...
// Cuadro de diálogo (misiones), abajo en el centro.
function createDialog() {
  const el = document.createElement("div");
  el.style.cssText = [
    "position:fixed", "bottom:110px", "left:50%", "transform:translateX(-50%)",
    "max-width:520px", "padding:12px 20px", "background:rgba(10,20,35,0.85)",
    "color:#fff", "font-family:Segoe UI, system-ui, sans-serif", "font-size:16px",
    "border:1px solid rgba(255,255,255,0.18)", "border-radius:12px",
    "text-align:center", "pointer-events:none", "user-select:none", "opacity:0",
    "transition:opacity 0.3s ease", "z-index:7",
  ].join(";");
  document.body.appendChild(el);
  return el;
}

// Aviso de emisora de radio (arriba a la izquierda, bajo el panel de controles).
function createRadioHud() {
  const el = document.createElement("div");
  el.style.cssText = [
    "position:fixed", "top:230px", "left:16px", "padding:6px 14px",
    "background:rgba(10,20,35,0.78)", "color:#ffd24a",
    "font-family:Segoe UI, system-ui, sans-serif", "font-size:14px", "font-weight:700",
    "border:1px solid rgba(255,255,255,0.15)", "border-radius:10px",
    "pointer-events:none", "user-select:none", "opacity:0",
    "transition:opacity 0.4s ease", "z-index:6",
  ].join(";");
  document.body.appendChild(el);
  return el;
}

// Nivel de búsqueda (estrellas, arriba a la derecha, bajo el reloj).

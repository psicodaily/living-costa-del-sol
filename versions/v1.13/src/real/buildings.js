...
export function buildBuildings(buildings) {
  ...
  for (const [color, geos] of byColor) {
    ...
    if (!merged) continue;
    const roofMat = new THREE.MeshStandardMaterial({ color: shade(color, 0.55), roughness: 0.95 });
    const wallMat = new THREE.MeshStandardMaterial({ color, map: windowTex, roughness: 0.9 });

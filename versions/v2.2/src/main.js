...
async function init() {
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); // tope 1.5 = más FPS
  ...
  const sky = createSky(scene, {
    radius: 6000,
    fogNear: 300,
    fogFar: 1700, // niebla más corta; los edificios lejanos se recortan
    shadowHalf: 220,
    shadowFar: 1400,
    shadowMapSize: 1024,
  });
  ...
  function animate() {
    ...
    for (const m of buildingMeshes) {
      ...
      const dx = c.x - camX;
      const dz = c.z - camZ;
      m.visible = dx * dx + dz * dz < 2050 * 2050;
    }

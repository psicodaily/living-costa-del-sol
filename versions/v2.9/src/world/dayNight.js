...
const DAY_LENGTH = 24 * 60; // 24 minutos reales = 1 día completo (estilo GTA)
const SUN_DIST = 200;
...
export function createDayNight({ sky, renderer, scene, windowMaterials = [], lampMaterials = [], showDiscs = true }) {
  ...
  if (showDiscs) {
    ...
    // En el mapa grande no mostramos discos de sol/luna (evita esferas flotantes).
  ...
  function apply() {
    ...
    // Ambiente y exposición (con suelo de luz para que los edificios no se vean negros).
    hemiLight.intensity = 0.35 + dayF * 0.6;
    renderer.toneMappingExposure = 0.85 + dayF * 0.35;

    // Disco del sol / halo (día) y luna (noche). Solo si showDiscs.
    if (showDiscs) {

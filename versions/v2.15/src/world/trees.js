...
export function createTrees(scene) {
  ...
  function place(x, z) {
    const mesh = rand() < 0.7 ? createPalm() : createRoundTree();
    mesh.position.set(x, 0, z);
    mesh.rotation.y = rand() * Math.PI * 2;
    group.add(mesh);
    list.push({ mesh, x, z, radius: 1.0, fallen: false, fallT: 0, fallDir: { x: 1, z: 0 } });
  }
  ...
  for (let i = 0; i <= CITY.blocks; i++) {
    for (let j = 0; j < CITY.blocks; j++) {
      ...
      if (rand() < 0.7) place(line, along + (rand() - 0.5) * 10);
      if (rand() < 0.7) place(along + (rand() - 0.5) * 10, line);
    }
  }

  scene.add(group);

  // Tumba un árbol en la dirección (dx,dz) del impacto.
  function knock(tree, dx, dz) {
    if (tree.fallen) return;
    const len = Math.hypot(dx, dz) || 1;
    tree.fallDir = { x: dx / len, z: dz / len };
    tree.fallen = true;
    tree.fallT = 0;
    tree.radius = 0; // ya no bloquea
  }

  // Anima la caída de los árboles tumbados.
  const _axis = new THREE.Vector3();
  function update(delta) {
    for (const t of list) {
      if (!t.fallen || t.fallT >= 1) continue;
      t.fallT = Math.min(1, t.fallT + delta * 2.2);
      _axis.set(t.fallDir.z, 0, -t.fallDir.x).normalize();
      t.mesh.setRotationFromAxisAngle(_axis, t.fallT * (Math.PI / 2));
    }
  }

  return { group, list, update, knock };
}
